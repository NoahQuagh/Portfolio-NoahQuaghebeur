<?php
session_start();
require_once __DIR__.'/../../../db/connexion_portfolio_db.php';

header('Content-Type: application/json; charset=utf-8');

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    http_response_code(405);
    echo json_encode(['success' => false, 'message' => 'Méthode non autorisée']);
    exit;
}

$parId = trim($_POST['parId'] ?? '');

if (empty($parId)) {
    echo json_encode(['success' => false, 'message' => 'Id du parcours manquant']);
    exit;
}

try {
    $pdo = getDB();

    $stmt = $pdo->prepare("delete from POR_PARCOURS where par_id=?");
    $stmt->execute([$parId]);

    header('Location: ../../../admin/admin_parcours.php');
    exit;

} catch (\Throwable $e) {
    error_log("[Delete Parcours Error] " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Erreur de suppréssion du parcours']);
}
